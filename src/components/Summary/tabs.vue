    ...
    <div>
        <TabGroup>
            <TabList class="mb-4 mt-7 flex gap-2">
                <Tab v-slot="{ selected }" as="template" v-for="(item, i) in headers" :key="i" @click="selectedIndex = i">
                    <button :class="[
            ...
            <div>
                <table class="w-full text-sm">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        <tr v-for="metric in data[selectedOption].data" :key="metric.name">
                            <td class="text-left font-semibold">{{ metric.name }}</td>
                            <td class="text-left">{{ roundValue(metric.value) }}</td>
